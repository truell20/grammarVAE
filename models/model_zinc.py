...
class MoleculeVAE():
    ...
    def _buildEncoder(self, x, f, latent_rep_size, max_length, max_length_func, epsilon_std = 0.01):
        ...
        def conditional(x_true, x_pred, max_l):
            ...
            M3 = tf.reshape(M2, [-1,max_l,DIM]) # reshape them
        ...
        def vae_loss(true, pred_decoded_mean):
            x_decoded_mean = conditional(true[0], pred_decoded_mean[0], max_length) # we add this new function to the loss
            x = K.flatten(true[0])
            x_decoded_mean = K.flatten(x_decoded_mean)
            xent_loss_1 = max_length * binary_crossentropy(x, x_decoded_mean)
            
            f_decoded_mean = conditional(true[1], pred_decoded_mean[1], max_length_functional) # we add this new function to the loss
            f = K.flatten(true[1])
